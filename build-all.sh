...
# build sundials four ways: (v7.5.0 and the archived branch with mrihh) x (Release and Logging)
...
make -j install
cd -
mkdir src-v7.5.0-logging/build
cd src-v7.5.0-logging/build
cmake ..
make -j install
cd -
