...
data=pd.read_excel('brusselator_full_results.xlsx')
bruss_failed_pairs = data.loc[data['ReturnCode'] == 1, ['control', 'mri_method']]
data2=pd.read_excel('kpr_full_results.xlsx')
...
if Plot_KPR:
    putil.do_comparison_plots('kpr_full_results.xlsx', 'omega', [50, 500], r'$\omega$', Full_controllers,'All controller',
                              'kpr-full',kpr_removed_pairs,'kpr_ranks',kpr_xlim_lo,kpr_xlim_mid,kpr_xlim_hi)

if Plot_Bruss:
    putil.do_comparison_plots(r"brusselator_full_results.xlsx", 'ep', [1.e-4, 1.e-5], r'$\epsilon$', Full_controllers, 'All controller',
...
if Plot_KPR:
   putil.print_failed_tests('kpr_decoupled_results.xlsx', 'KPR decoupled')
   putil.print_failed_tests('kpr_verification_results.xlsx', 'KPR verification')
   putil.print_failed_tests('kpr_comparison_results.xlsx', 'KPR comparison')
if Plot_Bruss:
   putil.print_failed_tests('brusselator_verification_results.xlsx', 'Stiff Brusselator verification')
   putil.print_failed_tests('brusselator_comparison_results.xlsx', 'Stiff Brusselator comparison')
