...
data_Bruss_SR32["zScore"] = data_Bruss_SR32["Controller"].map(methodAvg_Bruss_SR32)
data_Bruss_SR32["zScore"] = (data_Bruss_SR32["zScore"] - allAvg_Bruss_SR32)/allStd_Bruss_SR32

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_Bruss_SR32["zScore"] < -zscore_threshold,
    data_Bruss_SR32["zScore"] > zscore_threshold
]

data_Bruss_SR32["status"] = np.select(conditions, status, default="intermediate")
...
# ------------- Calculate the z-score of each controller and store on each row corresponding to the controller -----------------
data_Bruss_ERK33a["zScore"] = data_Bruss_ERK33a["Controller"].map(methodAvg_Bruss_ERK33a)
data_Bruss_ERK33a["zScore"] = (data_Bruss_ERK33a["zScore"] - allAvg_Bruss_ERK33a)/allStd_Bruss_ERK33a

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_Bruss_ERK33a["zScore"] < -zscore_threshold,
    data_Bruss_ERK33a["zScore"] > zscore_threshold
]

data_Bruss_ERK33a["status"] = np.select(conditions, status, default="intermediate")
...
# ------------- Calculate the z-score of each controller and store on each row corresponding to the controller -----------------
data_Bruss_ESDIRK34a["zScore"] = data_Bruss_ESDIRK34a["Controller"].map(methodAvg_Bruss_ESDIRK34a)
data_Bruss_ESDIRK34a["zScore"] = (data_Bruss_ESDIRK34a["zScore"] - allAvg_Bruss_ESDIRK34a)/allStd_Bruss_ESDIRK34a

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_Bruss_ESDIRK34a["zScore"] < -zscore_threshold,
    data_Bruss_ESDIRK34a["zScore"] > zscore_threshold
]

data_Bruss_ESDIRK34a["status"] = np.select(conditions, status, default="intermediate")
...
# ------------- Calculate the z-score of each controller and store on each row corresponding to the controller -----------------
data_Bruss_MERK32["zScore"] = data_Bruss_MERK32["Controller"].map(methodAvg_Bruss_MERK32)
data_Bruss_MERK32["zScore"] = (data_Bruss_MERK32["zScore"] - allAvg_Bruss_MERK32)/allStd_Bruss_MERK32

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_Bruss_MERK32["zScore"] < -zscore_threshold,
    data_Bruss_MERK32["zScore"] > zscore_threshold
]

data_Bruss_MERK32["status"] = np.select(conditions, status, default="intermediate")


# ----------------------------------- Store all data in one excel file with multiple sheets ---------------------------------
with pd.ExcelWriter('allOrder3_BrussControllers.xlsx') as writer:
...
# load excel file
filename = "allOrder3_BrussControllers.xlsx"
...
with open("AvgZscores_Bruss_Ord3.txt", "w") as file:
