...
data_KPR_ERK22b["zScore"] = data_KPR_ERK22b["Controller"].map(methodAvg_KPR_ERK22b)
data_KPR_ERK22b["zScore"] = (data_KPR_ERK22b["zScore"] - allAvg_KPR_ERK22b)/allStd_KPR_ERK22b

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_KPR_ERK22b["zScore"] < -zscore_threshold,
    data_KPR_ERK22b["zScore"] > zscore_threshold
]

data_KPR_ERK22b["status"] = np.select(conditions, status, default="intermediate")
...
# ------------- Calculate the z-score of each controller and store on each row corresponding to the controller -----------------
data_KPR_ERK22a["zScore"] = data_KPR_ERK22a["Controller"].map(methodAvg_KPR_ERK22a)
data_KPR_ERK22a["zScore"] = (data_KPR_ERK22a["zScore"] - allAvg_KPR_ERK22a)/allStd_KPR_ERK22a

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_KPR_ERK22a["zScore"] < -zscore_threshold,
    data_KPR_ERK22a["zScore"] > zscore_threshold
]

data_KPR_ERK22a["status"] = np.select(conditions, status, default="intermediate")
...
# ------------- Calculate the z-score of each controller and store on each row corresponding to the controller -----------------
data_KPR_IRK21a["zScore"] = data_KPR_IRK21a["Controller"].map(methodAvg_KPR_IRK21a)
data_KPR_IRK21a["zScore"] = (data_KPR_IRK21a["zScore"] - allAvg_KPR_IRK21a)/allStd_KPR_IRK21a

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_KPR_IRK21a["zScore"] < -zscore_threshold,
    data_KPR_IRK21a["zScore"] > zscore_threshold
]

data_KPR_IRK21a["status"] = np.select(conditions, status, default="intermediate")
...
# ------------- Calculate the z-score of each controller and store on each row corresponding to the controller -----------------
data_KPR_SR21["zScore"] = data_KPR_SR21["Controller"].map(methodAvg_KPR_SR21)
data_KPR_SR21["zScore"] = (data_KPR_SR21["zScore"] - allAvg_KPR_SR21)/allStd_KPR_SR21

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_KPR_SR21["zScore"] < -zscore_threshold,
    data_KPR_SR21["zScore"] > zscore_threshold
]

data_KPR_SR21["status"] = np.select(conditions, status, default="intermediate")
...
# ------------- Calculate the z-score of each controller and store on each row corresponding to the controller -----------------
data_KPR_RALSTON2["zScore"] = data_KPR_RALSTON2["Controller"].map(methodAvg_KPR_RALSTON2)
data_KPR_RALSTON2["zScore"] = (data_KPR_RALSTON2["zScore"] - allAvg_KPR_RALSTON2)/allStd_KPR_RALSTON2

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_KPR_RALSTON2["zScore"] < -zscore_threshold,
    data_KPR_RALSTON2["zScore"] > zscore_threshold
]

data_KPR_RALSTON2["status"] = np.select(conditions, status, default="intermediate")
...
# ------------- Calculate the z-score of each controller and store on each row corresponding to the controller -----------------
data_KPR_MERK21["zScore"] = data_KPR_MERK21["Controller"].map(methodAvg_KPR_MERK21)
data_KPR_MERK21["zScore"] = (data_KPR_MERK21["zScore"] - allAvg_KPR_MERK21)/allStd_KPR_MERK21

# ----------------------------- Classify each method based on z-score ------------------------------------
conditions = [
    data_KPR_MERK21["zScore"] < -zscore_threshold,
    data_KPR_MERK21["zScore"] > zscore_threshold
]

data_KPR_MERK21["status"] = np.select(conditions, status, default="intermediate")


# ----------------------------------- Store all data in one excel file with multiple sheets ---------------------------------
with pd.ExcelWriter('allOrder2_KPRcontrollers.xlsx') as writer:
    data_KPR_ERK22b.to_excel(writer, sheet_name='data_KPR_ERK22b', index=False)
    data_KPR_ERK22a.to_excel(writer, sheet_name='data_KPR_ERK22a', index=False)
    data_KPR_IRK21a.to_excel(writer, sheet_name='data_KPR_IRK21a', index=False)
    data_KPR_SR21.to_excel(writer, sheet_name='data_KPR_SR21', index=False)
    data_KPR_RALSTON2.to_excel(writer, sheet_name='data_KPR_RALSTON2', index=False)
    data_KPR_MERK21.to_excel(writer, sheet_name='data_KPR_MERK21', index=False)


###############################################################################################################################
# --------------------------------------- Compute the Average z-score of Each Controller ------------------------------------------
# load excel file
filename = "allOrder2_KPRControllers.xlsx"
...
with open("AvgZscores_KPR_Ord2.txt", "w") as file:
